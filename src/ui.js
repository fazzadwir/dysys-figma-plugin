...
function setPlatform(p) {
  ...
  document.getElementById('btn-mobile').classList.toggle('active', p === 'mobile');
  renderTypoPreview();
}
...
function generatePalette(hex) {
  ...
  const lightnessMap = {
    ...
    500: 45, 600: 36, 700: 27, 800: 18, 900: 11, 950: 7,
  };
  const palette = {};
...
function renderShadeList(baseHex) {
  ...
  const colorName = (colorNameInput.value.trim() || 'Color').replace(/\s+$/, '');

  if (!isValidHex(baseHex)) {
    list.innerHTML = '<div class="shade-empty-hint">Enter a valid HEX color above to preview shades.</div>';
    return;
  }
  ...
  list.innerHTML = shadeSorted.map(shade => {
...
function onShadeHSLChange(shade) {
  ...
  const hex = hslToHex(shadeState[shade].h, shadeState[shade].s, shadeState[shade].l);
...
function onShadeRGBChange(shade) {
  const r = Math.min(255, Math.max(0, parseInt(document.getElementById('shade-r-' + shade).value) || 0));
  const g = Math.min(255, Math.max(0, parseInt(document.getElementById('shade-g-' + shade).value) || 0));
  const b = Math.min(255, Math.max(0, parseInt(document.getElementById('shade-b-' + shade).value) || 0));
  const hexVal = '#' + [r, g, b].map(x => x.toString(16).padStart(2, '0')).join('');
  const [h, s, l] = hexToHsl(hexVal);
  if (!shadeState[shade]) shadeState[shade] = { h: Math.round(h), s: Math.round(s), l: Math.round(l), format: 'RGB' };
  shadeState[shade].h = Math.round(h);
  shadeState[shade].s = Math.round(s);
  shadeState[shade].l = Math.round(l);
          ...
          <div class="style-val">
            <select class="typo-weight-select" onchange="updateTypoWeight(${i}, parseInt(this.value))">
              ${[300, 400, 500, 600, 700, 800, 900]
                .map(w => `<option value="${w}" ${r.weight === w ? 'selected' : ''}>${w}</option>`)
                .join('')}
            </select>
          </div>
...
const GOOGLE_FONTS = [
  ...
  { name: "Nunito", cat: "Sans Serif" }, { name: "Raleway", cat: "Sans Serif" },
  { name: "Rubik", cat: "Sans Serif" }, { name: "Outfit", cat: "Sans Serif" },
  { name: "DM Sans", cat: "Sans Serif" }, { name: "Plus Jakarta Sans", cat: "Sans Serif" },
  { name: "Figtree", cat: "Sans Serif" }, { name: "Manrope", cat: "Sans Serif" },
  { name: "Work Sans", cat: "Sans Serif" }, { name: "Mulish", cat: "Sans Serif" },
  { name: "Karla", cat: "Sans Serif" }, { name: "Jost", cat: "Sans Serif" },
  { name: "Lexend", cat: "Sans Serif" }, { name: "Geist", cat: "Sans Serif" },
  { name: "Source Sans 3", cat: "Sans Serif" }, { name: "Barlow", cat: "Sans Serif" },
  { name: "IBM Plex Sans", cat: "Sans Serif" }, { name: "Urbanist", cat: "Sans Serif" },
  { name: "Sora", cat: "Sans Serif" }, { name: "Space Grotesk", cat: "Sans Serif" },
  { name: "Noto Sans", cat: "Sans Serif" }, { name: "Quicksand", cat: "Sans Serif" },
  { name: "Oxanium", cat: "Sans Serif" }, { name: "Josefin Sans", cat: "Sans Serif" },
  { name: "Playfair Display", cat: "Serif" }, { name: "Merriweather", cat: "Serif" },
  { name: "Lora", cat: "Serif" }, { name: "EB Garamond", cat: "Serif" },
  { name: "Cormorant Garamond", cat: "Serif" }, { name: "DM Serif Display", cat: "Serif" },
  { name: "Libre Baskerville", cat: "Serif" }, { name: "Source Serif 4", cat: "Serif" },
  { name: "IBM Plex Serif", cat: "Serif" }, { name: "Noto Serif", cat: "Serif" },
  { name: "Bitter", cat: "Serif" }, { name: "Crimson Text", cat: "Serif" },
  { name: "Fraunces", cat: "Serif" }, { name: "Spectral", cat: "Serif" },
  { name: "Alegreya", cat: "Serif" }, { name: "Fira Code", cat: "Monospace" },
  { name: "Source Code Pro", cat: "Monospace" }, { name: "JetBrains Mono", cat: "Monospace" },
  { name: "IBM Plex Mono", cat: "Monospace" }, { name: "Space Mono", cat: "Monospace" },
  { name: "Roboto Mono", cat: "Monospace" }, { name: "Courier Prime", cat: "Monospace" },
...
function renderFontList(filter = '') {
  ...
  const filtered = filter
    ? GOOGLE_FONTS.filter(f => f.name.toLowerCase().includes(filter.toLowerCase()))
    : GOOGLE_FONTS;
...
function toggleFontDropdown() {
  ...
  document.getElementById('font-dropdown').classList.toggle('open', dropdownOpen);
  document.getElementById('font-chevron').classList.toggle('open', dropdownOpen);
  if (dropdownOpen) {
...
function filterFonts(val) { renderFontList(val); }
...
btnTypo.addEventListener('click', () => {
  ...
  parent.postMessage(
    { pluginMessage: { type: 'generate-typography', fontFamily: font, platform, styles: rows } },
    '*'
  );
});
...
(function () {
  ...
  window.setGridRule = function(px) {
    ...
    document.getElementById('spacing-empty-state').querySelector('.spacing-empty-title').textContent = 'Select a frame to start';
    document.getElementById('spacing-empty-state').querySelector('.spacing-empty-desc').innerHTML =
  ...
  function sendScan()           { parent.postMessage({ pluginMessage: { type: 'scan', grid: gridRule } }, '*'); }
  ...
  function renderSpacingResults(records, grid) {
    ...
    spSummaryEl.innerHTML = `
      <div class="sp-stat-card">
        ...
        <div class="sp-stat-label">Spacing Found</div>
      </div>
      <div class="sp-stat-card">
      ...
      <div class="sp-stat-card" style="grid-column: span 2">
        <div style="display:flex;align-items:center;gap:6px">
          ...
          <div class="sp-stat-label" style="text-transform:none;font-size:11px">grid rule — scanned all descendants recursively</div>
        </div>
      </div>
    `;
